...
const getRadomImages = (nbrOfImages) => {
  ...
  for (var i = 0; i < nbrOfImages; i++) {
    ...
    while (imagesArray.indexOf(id) !== -1 || playedImages.indexOf(id) !== -1) {
...
<template>
  <main>
    <ul class="players">
      <li v-for="player in players" :class="{ active: currentUserId == player.id }">
    ...
    <template v-else v-for="player in players">
      <div v-if="currentUserId == player.id">
        ...
        <div v-else class="footer">
          ...
          <button v-if="gameState == 'hint' && player.selected" @click="progressGame()">
            Vidare
          </button>
          <button v-else-if="gameState == 'vote' && player.vote" @click="progressGame()">
            Rösta
          </button>
          <p v-else class="small">Välj en bild för att komma vidare...</p>
        </div>
      </div>
    </template>
  </main>
